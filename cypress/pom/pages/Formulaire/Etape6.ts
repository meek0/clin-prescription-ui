...
const selectors = {
  status: (status: string) => `[id="father_father_status"] input[value="${statusValues(status)}"]`,
  ...
  inputRamq: '[id="father_father_jhn"]',
  loupeRamq: '[id="father_father_jhn"] button[class*="ant-input-search-button"]',
  inputNom: '[id="father_father_last_name"]',
  inputPrenom: '[id="father_father_first_name"]',
  affected: (affected: string) => `[id="father_father_parent_clinical_status"] input[value="${affectedValues(affected)}"]`,
  ...
  ageJeuneAdulte: '[class="rc-virtual-list"] [data-cy="SelectOptionHP:0011462"]',
  commentaireClinique: 'textarea[id="father_father_comment"]',
...
export const Etape6 = {
  actions: {
    ...
    selectAgeJeuneAdulte(eq: number) {
      cy.get(selectors.selectAge).eq(eq).clickAndWait();
      cy.get(selectors.ageJeuneAdulte).filter(':visible').clickAndWait({force: true});
    },
